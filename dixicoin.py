...
NODE_LIST = urlopen('https://pastebin.com/raw/YBCuR2Qt').read()
...
def update_system():
    ...
    run_command('sudo DEBIAN_FRONTEND=noninteractive apt-get -yq -o DPkg::options::="--force-confdef" -o DPkg::options::="--force-confold"  install grub-pc')
...
def compile_wallet():
    ...
    print_info("Downloading wallet...")
    run_command("wget https://github.com/Dixicoin-DXC/Dixicoin/releases/download/untagged-9b6fe8bd210320b07944/dixi-4.3.0-aarch64-linux-gnu.zip")
    run_command("unzip dixi-4.3.0-aarch64-linux-gnu.zip -d dixicoin")
    run_command("mv dixicoin/dixi-4.3.0-aarch64-linux-gnu/dixicoind /usr/local/bin/dixicoind")
    run_command("mv dixicoin/dixi-4.3.0-aarch64-linux-gnu/dixicoin-cli /usr/local/bin/dixicoin-cli")
    run_command("chmod +x /usr/local/bin/dixicoind")
    run_command("chmod +x /usr/local/bin/dixicoin-cli")
    run_command("rm dixi-4.3.0-aarch64-linux-gnu.zip")		

def get_total_memory():
    return (os.sysconf('SC_PAGE_SIZE') * os.sysconf('SC_PHYS_PAGES'))/(1024*1024)

def autostart_masternode(user):
    job = "@reboot /usr/local/bin/dixicoind\n"
...
def setup_first_masternode():
    ...
    os.system('su - mn1 -c "{}" '.format("dixicoind -daemon &> /dev/null"))

    print_info("Open your desktop wallet config file (%appdata%/Dixicoin/dixicoin.conf) and copy your rpc username and password! If it is not there create one! E.g.:\n\trpcuser=[SomeUserName]\n\trpcpassword=[DifficultAndLongPassword]")
    global rpc_username
    global rpc_password
...
mnconflock=1
masternode=1
masternodeaddr={}:61150
masternodeprivkey={}
{}""".format(rpc_username, rpc_password, SERVER_IP, masternode_priv_key, NODE_LIST)

    print_info("Saving config file...")
    run_command('su - mn1 -c "{}" '.format("mkdir -p /home/mn1/.dixicoin/ && touch /home/mn1/.dixicoin/dixicoin.conf"))
    f = open('/home/mn1/.dixicoin/dixicoin.conf', 'w')
    f.write(config)
    f.close()
...
#    run_command('su - mn1 -c "{}" '.format("cd && unzip -d .dixicoin -o " + filename))

    run_command('rm /home/mn1/.dixicoin/peers.dat') 
...
IP: {}
Private key: {}
Transaction ID: [1k desposit transaction id. 'masternode outputs']
Transaction index: [1k desposit transaction index. 'masternode outputs']
--------------------------------------------------
"""
